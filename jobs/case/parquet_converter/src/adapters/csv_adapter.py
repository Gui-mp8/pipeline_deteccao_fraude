...
class CsvAdapter(ParquetTransformer):
    """Adapter that reads CSV from landing storage and transforms it to Parquet."""
    ...
    def read_from_storage(self, source_uri: str) -> Iterator[dict[str, object]]:
        if not source_uri.startswith("gs://"):
            raise ValueError("CSV source_uri must be a Cloud Storage URI starting with gs://")
